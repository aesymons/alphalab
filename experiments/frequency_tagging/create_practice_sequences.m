...
attended_dim = 3; % 1 = pitch, 2 = duration, 3 = passive 
dim1changes = 3;
dim2changes = 2;
...
for block = 1:num_blocks
    ...
    path2sequences= [cd '/practice_sequences/'];
    stim_name = strcat(path2sequences,['practice_' dimensions{attended_dim} '_pitch' num2str(dim1changes) '_' num2str(block)]);  
    timings_text = fopen(strcat(stim_name,'.json'),'w');
    writetable(info,strcat(stim_name,'.csv'));
    fprintf(timings_text,timings);
    audiowrite(strcat(stim_name,'.wav'), stimulus, 44100);
