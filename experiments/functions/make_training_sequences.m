function [info fnames] = make_training_sequences(tone_rate,duration,nlevels,ntrials,f0_changes,dur_changes,f0_rep,dur_rep,inputPath,outputPath,format)
...
f0_order = randsample(repelem([1:nlevels],nlevels),ntrials);
dur_order = randsample(repelem([1:nlevels],nlevels),ntrials);
...
f0_order = randsample([1 1 2 2],ntrials);
dur_order = randsample([1 1 2 2],ntrials);
...
for n = 1:ntrials
    ...
    if f0_changes > 0
        ...
        f0_seq = repmat(f0_order(n),stimlength,1);
    end
    ...
    if dur_changes > 0
        ...
        dur_seq = repmat(dur_order(n),stimlength,1);
    end
    ...
    filename = [outputPath 'training_tones_pitch' num2str(f0_changes) '_dur' num2str(dur_changes) '_' num2str(f0_rep) '_' num2str(dur_rep) '_' num2str(n) '.flac'];
